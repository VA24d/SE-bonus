...
The Software Carbon Intensity (SCI) specification provides a standard methodology for scoring the carbon footprint of software applications. SCI measures carbon emissions per functional unit, factoring in operational energy (compute, storage, and network) and the embodied emissions of the underlying hardware. Reducing the CPU and network overhead directly lowers the operational energy component of the SCI score \cite{b1, b2}.
...
Message brokers exhibit different resource profiles based on their architecture. Benchmark reports indicate that Kafka achieves much higher throughput than RabbitMQ, but with higher baseline resource use. Kafka can sustain roughly 76,000--100,000+ messages per second compared to 7,000--18,000 msg/s for RabbitMQ \cite{b5}. Consequently, Kafka's theoretical CPU usage per message is lower at massive scales. RabbitMQ is resource-efficient for low-volume scenarios due to its in-memory nature, but Kafka becomes substantially more efficient per-message at scale due to sequential disk I/O and batching.
...
\bibitem{b1} R. Mahmoud and A. Qusef, ``A Systematic Literature Review on Green Software Engineering,'' \textit{IEEE Access}, vol. 10, pp. 8652--8664, 2022.
\bibitem{b2} Green Software Foundation, ``Software Carbon Intensity (SCI) Specification,'' 2021. [Online]. Available: \url{https://greensoftware.foundation/sci}. [Accessed: 30-Apr-2026].
